...
const DormSlider: React.FC<DormSliderProps> = ({ dormList, selectedDorm }) => {
  ...
  useEffect(() => {
    if (selectedDorm && swiperRef.current) {
      ...
      const index = visibleDorms.findIndex(
        (dorm) => dorm.name === selectedDorm
      );
      console.log(index);
      if (index !== -1) {
            ...
            <SwiperSlide key={index} className="relative">
              ...
              <div className="">
                <div className="flex items-center">
                  ...
                  <span className="text-slate-500 ml-1 text-[12px]">
                    {dorm.address.split(" ").slice(0, 4).join(" ")}
                  </span>
                </div>
                <span className="mt-4 text-[20px] font-semibold">{`${dorm.price}`}</span>
                <span className="text-[14px] ml-2 text-slate-500">/month</span>
              </div>
            </SwiperSlide>
          ) : null
        )}
