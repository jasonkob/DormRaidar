...
export default function Dorm (){
    ...
    const dormReview =[
        {username:"juthamat" ,point:3 ,date:"14/02/25" ,comment:"เหมือนจะรีโนเวทห้องใหม่นะคะ ห้องดูใหม่มากๆ ขนาดสียังไม่แห้งเลยค่ะ"},
        {username:"worachit" ,point:4 ,date:"24/02/25" ,comment:"ห้องน้ำมีแบ่งโซนเปียกให้ด้วยครับ ดีมากเลย"},
    ]
                ...
                <>  
                    ...
                    <section className=" w-full h-1/2 flex justify-center ">
                        ...
                        <div className="w-2/6 h-full mx-2 ">
                            <div className="w-full h-1/2 relative ml-1">
                                <Image src="/roomExam1.png" alt="more Image" fill ></Image>
                            </div>
                            <div className="flex w-full h-full mt-2">
                                <div className="w-full h-1/2 relative mr-1 ml-2">
                                    <Image src="/roomExam2.png" alt="more Image" fill ></Image>
                                </div>
                                <div className="w-full h-1/2 relative ml-1 ">
                                    <div className="absolute inset-0 flex items-center justify-center  text-xl font-bold z-10">
                                        +2
                                    </div>
                                    <Image src="/roomExam3.png" alt="more Image" fill className="opacity-40"></Image>
                                </div>
                            </div>
                        </div>
                    ...
                    <section className="w-full   flex mx-5 my-3">
                        ...
                        <div className="w-4/6 mr-2 mb-3">
                            <div className="text-[22px] font-semibold" >{filteredDorms[0].name}</div>
                            ...
                            <div className="mt-8"> 
                                <span className="text-[20px] font-semibold">สิ่งอำนวยความสะดวก</span>
                                <div className="mt-4">
                                    {filteredDorms[0].convenience.map((data,index) => (
                                        <span key={index} className="p-3 mr-3 bg-indigo-400 text-white rounded-lg select-none">{data}</span>
                                    ))}
                                </div>
                            </div>

                            <div className="mt-6">
                                <span  className="text-[20px] font-semibold">กฎระเบียบหอพัก</span>
                                <ul >
                                    {dormRule.map((rule,index) => (
                                        <li key={index} className="list-disc pl-3 ml-6 mb-1 mt-1"> {rule}</li>
                                    ))}
                                </ul>
                            </div>
                                    ...
                                    {dormReview.map((review,index)=> (
                                        <li key={index} className="my-2 mt-1 mb-4">
                                            ...
                                            <div className="flex mb-2 mt-2 pl-3">
                                                {Array.from({length:review.point}).map((_,index) => (
                                                    <svg key={index} width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                        <path d="M5.825 21L7.45 13.975L2 9.25L9.2 8.625L12 2L14.8 8.625L22 9.25L16.55 13.975L18.175 21L12 17.275L5.825 21Z" fill="#F1CF61"/>
                                                    </svg>
                                                ))}
                                                <span className="ml-3 text-slate-400">{review.date}</span>
                                            </div>
                        ...
                        <div className="w-2/6 relative ml-2 mt-2">
                            <div className="w-11/12 absolute top-0  flex flex-col border border-slate-300 text-center rounded-xl items-center">
                                <span className="text-[20px] font-semibold mt-2" >รายละเอียดห้องพัก</span>
                                <div className="w-full mt-3">
                                    ...
                                    <div className="flex justify-between px-4 py-2 mt-2 mx-2 border-b">
                                        <span>ค่าเช่า</span>
                                        <span>{filteredDorms[0].price} บาท <span className="text-[14px] text-slate-400">/เดือน</span></span>
                                    </div>
                                    <div className="flex justify-between px-4 py-2 mt-2 mx-2 border-b">
                                    ...
                                    <div className="flex justify-between px-4 py-2 mt-2 mx-2 border-b">
                                        <span>ค่าไฟ</span>
                                        <span>8 บาท <span className="text-[14px] text-slate-400">/หน่วย</span></span>
                                    </div>
                                    <div className="flex justify-between px-4 py-2 mt-2 mx-2 border-b mb-2">
                                        <span>ค่าน้ำ</span>
                                        <span>18 บาท <span className="text-[14px] text-slate-400">/หน่วย</span></span>
                                    </div>
                                </div>
                                <Link href="/payment" className="w-11/12 bg-yellow-400 text-[20px] p-2 rounded-lg my-5 mb-10 transition duration-200 ease-in-out shadow-lg hover:bg-yellow-300">
                                    จองหอพัก
                                </Link>
                            </div>
                        </div>
