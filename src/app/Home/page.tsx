    ...
    return(
        <div className="bg-[url('/home.jpg')]  bg-cover bg-center h-screen w-full rounded-3xl opacity-90">
            <div className="flex h-1/4 w-full justify-center items-end">
                <div className='flex items-center bg-white px-4 pt-4  h-[110px] w-[900px] rounded-xl opacity-90'>
                    <section className='flex flex-col h-full w-4/12 '>
                        <label htmlFor="Location" className='text-[24px] font-bold mb-2 select-none '>Location</label>
                        <div className='w-10/12 flex items-center border border-black rounded-md mb-3 cursor-pointer' onClick={showLocation}>
                            ...
                            <span className="w-3/5 select-none">{location}</span>
                            <span className='ml-1' >
                            ...
                            <div className='bg-white border rounded-md w-10/12 '>
                                <ul className=''>
                                    {allLocation.map((item,index)=>(
                                        <li className='p-2 rounded-md pl-4 cursor-pointer select-none transition duration-100 ease-in hover:bg-yellow-200' key={index} onClick={()=>selectLocation({location:item.content})}>{item.content}</li>
                                    ))}
                                </ul>
                            </div>
                        )}
                            ...
                            <div className='bg-white border rounded-md w-10/12'>
                                <ul className=''>
                                    {allPrice.map((item,index)=>(
                                        <li className='p-2 rounded-md pl-4 cursor-pointer select-none transition duration-100 ease-in hover:bg-yellow-200' key={index} onClick={()=>selectPrice({price:item.price})}>{item.price}</li>
                                    ))}
                                </ul>
                            </div>
                        )}
            ...
            <div className='h-4/6 w-8/12 flex flex-col place-self-end items-center justify-center text-center overflow-hidden'>
                <p className='text-[28px] text-white text-center mt-20'>Discover your perfect dorm —</p>
                <p className='text-[18px] text-white self-center'>your smart choice for a comfortable and connected <br/> student life!</p>
            </div>
        </div>  
    )
}
